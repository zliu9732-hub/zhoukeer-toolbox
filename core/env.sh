...
config_key_is_allowed() {
    case "$1" in
        TOOLBOX_NAME|DUAL_BOOT_TIMEOUT|GITHUB_RELEASE_PROXY|GITHUB_MIRRORS|GITHUB_PROBE_CONNECT_TIMEOUT|GITHUB_PROBE_MAX_TIME|GITHUB_CONNECT_TIMEOUT|GITHUB_MAX_TIME|GITHUB_RETRIES|GITHUB_MIN_SPEED_BYTES|GITHUB_MIN_SPEED_TIME|GITHUB_DOWNLOAD_PROXY|DECKY_LOADER_URL|DECKY_LOADER_SHA256|DECKY_SERVICE_URL|DECKY_SERVICE_SHA256|DECKY_LSFG_URL|DECKY_LSFG_SHA256|DECKY_FSR4_URL|DECKY_FSR4_SHA256|DECKY_CHEATDECK_URL|DECKY_CHEATDECK_SHA256|DECKY_TOMOON_URL|DECKY_TOMOON_SHA256|DECKY_DECKRECALL_URL|DECKY_DECKRECALL_SHA256|DECKY_SAVEPULSE_URL|DECKY_SAVEPULSE_SHA256|DECKY_SAVEPULSE_VERSION|DECKY_SIMPLE_TDP_URL|DECKY_SIMPLE_TDP_VERSION|DECKY_SIMPLE_TDP_SHA256|DECKY_UNIFIDECK_URL|DECKY_UNIFIDECK_VERSION|DECKY_UNIFIDECK_SHA256|GE_PROTON_URL|GE_PROTON_VERSION|GE_PROTON_SHA256|DECKY_FREEDECK_URL|DECKY_FREEDECK_SHA256|DECKY_FREEDECK_VERSION|DECKY_GITEE_ARCHIVE_URL|DECKY_GITEE_ARCHIVE_SHA256|DECKY_GITEE_ARCHIVE_PREFIX|DECKY_DOWNLOAD_PROXY) return 0 ;;
        *) return 1 ;;
    esac
}

config_key_is_retired_silently() {
    case "$1" in
        DECKY_LSFG_ZH_URL|DECKY_LSFG_ZH_SHA256|DECKY_FSR4_ZH_URL|DECKY_FSR4_ZH_SHA256) return 0 ;;
        *) return 1 ;;
    esac
}
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ "$line" =~ ^([A-Z][A-Z0-9_]*)=(.*)$ ]]; then
            ...
            if ! config_key_is_allowed "$key"; then
                config_key_is_retired_silently "$key" && continue
                printf '忽略未支持的配置项：%s\n' "$key" >&2
                continue
            fi
