            ...
            lsfg)
                ...
                case "$lsfg_choice" in
                    stable)
                        ...
                        ;;
                    mako)
                        gui_confirm "MAKO 小黄鸭将跟随 eugeniosegala/MAKO 最新正式版，优先从 Gitee mirror-3 下载，镜像尚未同步时回退作者 GitHub Release；全程校验作者 SHA256，并保留作者与 GPL-3.0-or-later 许可证。是否继续？" && \
            ...
            decky-install)
                ...
                case "$decky_choice" in
                    ...
                    test)
                        ...
                        ;;
                    rog-white-install)
                        gui_confirm "将 Renkit 内置的 ROG White v1.4.8 白色主题放入 CSS Loader 主题目录。需要已安装主题美化（CSS Loader），安装后请在 CSS Loader 中开启。是否继续？" && \
