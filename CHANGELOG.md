...
- 修复三款常用功能插件仍可能沿用失效 123 地址的问题：新的和历史的工具箱 123 配置都会迁移到插件作者固定 GitHub Release。
...
- 小黄鸭、Decky-Framegen 和 CheatDeck 改为作者 GitHub Release 固定版本并校验 SHA256；旧版工具箱写入的默认 123 地址会在运行时自动迁移，用户自定义地址不受影响。
...
- Firefox 改为从官方 Flathub 安装 `org.mozilla.firefox`，不再使用123云盘或国内镜像，后续更新交由 Flatpak 管理。
