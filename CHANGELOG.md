...
## V6.0.0 正式版 — 2026-07-30

- 全面核对触控与 GUI 菜单，修复 GUI 卸载第 4 页不可达、二级页侧栏导航失效、插件名称与说明错位、AnyDesk 卸载缺失及两套界面确认流程不一致。
- 统一 V6 品牌与简约视觉：保留原字体、120×32 画布和触控坐标，降低背景干扰并修复底部提示覆盖按钮触控区。
- GitHub Release 与标签归档在未启用 Steam302 时优先尝试 `ghfast.top`；测速失败的白名单源仍进入完整下载回退，修复重复测速缓存，并让 Decky 官方备用下载复用同一校验链。
- 修复自动更新可能降级、首次安装版本错位、诊断脱敏格式、网络名称解析超时、0% 电量预检、系统分区空间遗漏、国内源标记异常和新版 Flatpak 缓存入口不一致。
- 更新包排除测试、网站和开发文件，继续保留 HTTPS、超时、大小、SHA256、包内版本、归档结构和原子替换检查。
- 本版为周克儿工具箱 V6 正式版；后续维护范围仅限新增插件或维护 GitHub 镜像源。
