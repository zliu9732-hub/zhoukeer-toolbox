## Renkit 2.7.1 连续下载进度 — 2026-09-14

- 软件、插件、启动器、国内镜像和 Renkit 自身更新统一显示单行 20 格进度条、准确百分比与括号内实时速度。
- 自动识别并丢弃 HTTP 跳转页的伪 100% 帧，正式文件下载过程中百分比只增不减，不再出现 100% 后倒退到 5% 或 10%。
- 下载主进程会等待最后一帧和换行彻底写完才进入下一项，避免相邻任务的进度条上下重叠；元数据查询继续静默。

## Renkit 2.7.0 新机初始化不再等待 Discover 索引 — 2026-09-14

- Discover 系统组件仍在“初始化国内源并检测系统组件”中同步修复，确保版本一致后才继续。
- 新机初始化会把 Flatpak 用户仓库修复、AppStream 刷新、KDE 缓存重建和 Discover 启动延后到全部安装结束，再转入后台执行；既不阻塞后续步骤，也不与 Flatpak 软件安装争用仓库锁。
- Flatpak 仓库修复限制为 90 秒，AppStream 刷新限制为 120 秒；单项超时后仍会继续尝试重建缓存和启动商店。
...
- MAKO 安装流程不再删除旧版 LSFG-VK 目录；新增回归测试防止两版互相覆盖或清理。
- 新机初始化仍会额外安装 MAKO 官方简体中文包。

## Renkit 2.5.9 新机初始化加入 MAKO 小黄鸭 — 2026-09-12

- 新机初始化现在会安装 MAKO 小黄鸭官方简体中文包，并在最终状态清理旧版 LSFG-VK 目录，只保留 MAKO。
- 初始化确认页、图形提示和桌面交付说明已明确标注 MAKO 小黄鸭。
- 新增模拟流程断言，防止后续改动遗漏 MAKO 安装步骤。
...
- 国内镜像继续校验固定 SHA256、文件大小和分块清单；任何校验失败都会保留原系统或已安装版本。
- 新增独立自动同步模式，代码更新后先把固定资产上传至 Gitee，再发布 Renkit 客户端更新。

## Renkit 2.5.5 下载进度实时显示 — 2026-09-11

- 修复 curl 的回车进度帧被管道缓冲、导致下载完成后才出现进度条的问题；现在下载过程中会即时刷新百分比和速度。
- 新增流式回归测试，确认上游下载尚未结束时首帧进度已经可见。

## Renkit 2.5.3 下载提示统一 — 2026-09-11

- SteamOS 菜单、GUI、插件安装状态、Clover 状态和 Decky 独立安装器统一显示“国内镜像”，不再向用户显示内部文件拆分方式。

## Renkit 2.5.2 插件与附加资源下载进度 — 2026-09-11

- FSR4、小黄鸭、SteamDB 游戏数据、沉浸式翻译等完整插件包不再隐藏国内镜像下载进度。
- 游戏启动器封面资源下载也接入统一单行进度条，避免等待时没有反馈。
- 安装插件商城及插件组合时会读取已安装版本，并按 SteamOS 通道更新到镜像清单中的最新版；仅版本一致时跳过。

## Renkit 2.5.1 多文件下载单行刷新 — 2026-09-11

- GE-Proton、ToDesk、Decky 与预装启动器等多文件下载在中间文件完成时不再换行，整个包完成后才换行。
- 下载失败仍立即结束当前进度行并显示错误，避免错误提示黏在进度条后面。

## Renkit 2.5.0 单行动态下载进度 — 2026-09-11

- 所有使用统一下载进度的功能在 Konsole 中只刷新同一行，不再把每次百分比更新展开为多行。
- GUI 日志不再重复记录每一帧进度，百分比和实时速度仍在终端持续更新。

## Renkit 2.4.9 统一下载进度条 — 2026-09-11

- 软件、插件、启动器、国内大文件和 Renkit 更新包下载统一显示 20 格进度条、百分比与实时速度。
- Decky PluginLoader 下载使用连续的总体百分比，不显示内部传输编号或数量。

## Renkit 2.4.8 Decky 下载文案修复 — 2026-09-11

- 测试版插件商城下载时只显示统一的 `Decky PluginLoader` 名称与实时速度，不再展示内部传输编号或数量。

## Renkit 2.4.7 Switch to Windows 安装修复 — 2026-09-11

- 修复 Renkit 2.4.6 更新时安装器未把 `Switch to Windows` 运行组件复制到正式安装目录，导致点击安装后提示“本地插件组件不完整”的问题。
- 安装器现在会校验并复制插件清单、前端、后端入口与许可证；缺少任一运行文件时停止更新并保留旧版本。
- 新增模拟更新回归测试，确认正式安装目录包含插件所需的全部六项运行组件。
- 修复测试版 Decky PluginLoader 下载阶段长时间无反馈的问题：国内线路改为直接下载，显示实时进度并继续执行 SHA256 校验；只有国内镜像失败后才回退官方 Release。
...
- “掌机功耗控制”汉化版同步上游 SimpleDeckyTDP v1.0.7，纳入带地区后缀语言码（例如 `en-US`）的识别修复，并继续保留“掌机功耗控制”中文名称、完整简体中文界面及 `RenAmamiya` 汉化署名。
- 官方完整包、固定 SHA256、国内镜像清单、GitHub Release 回退和内置汉化组件统一升级到 v1.0.7；安装仍按“国内镜像校验 → GitHub 回退 → 同版本汉化覆盖”的原有安全流程执行，未来上游新版本未完成汉化时不会抢跑覆盖固定基线。
- 中文版界面强制使用简体中文，并将会覆盖汉化的插件内官方重装入口替换为“请在 RenKit 中更新”的中文提示，避免日志显示安装了汉化版、随后又被官方 OTA 重装成英文版。
- SteamOS 原 `/home/swapfile` 除不可变属性外若还带只追加属性，Renkit 会在停用 swap 后临时解除两种文件保护再原子替换；发生回滚时恢复原保护，仍无法移动时继续保留系统文件并安全使用独立 swap。
...
- 分类菜单只允许当前实际绘制的按钮接收触摸，未显示条目与卡片间空白不再触发动作；SteamOS 与 Bazzite 共用修复，不修改界面样式、插件安装或确认流程。
- 使用两套真实分页函数进行隔离测试，覆盖末页 1～5 项、三种终端尺寸、按钮文字与边角、空白区域，以及连续翻页、返回首页和宽窄窗口切换；安装入口均替换为禁止执行的模拟函数。
- 保留新机准备页、启动器日志过滤、分段触摸和八种终端尺寸回归；掌机触屏硬件与终端驱动仍需实机验证。

## Renkit 2.2.7 新机初始化准备页显示修复 — 2026-09-01

- 修复点击“新机初始化”后仍停留在旧画面的问题：准备页在读取动作时补绘，无换行的终端画面被启动器按行处理的 stderr 日志过滤阻塞。
- 界面绘制时保留独立终端输出，准备页和窗口缩放重绘直接送到终端，不再进入日志过滤；动作返回值仍只包含动作 ID，无终端调用保留原 stderr 回退。
- 不修改初始化安装流程、开发者模式与 CEF 远程调试确认要求，也不改变菜单布局或颜色。
- 使用真实的新机菜单、准备页和启动器过滤函数进行隔离回归，覆盖进入、返回、缩放及确认后仅分派一次模拟初始化；不执行任何真实安装或系统修改。
- 全部 Shell 语法及触控、菜单、启动器和八种终端尺寸模拟测试通过；仍需掌机实机确认。

## Renkit 2.2.6 侧栏自适应与触控恢复 — 2026-09-01

- 保留左侧贯通竖框和逐项横线分隔，菜单均分窗口可用高度，底边延伸至页脚上方，减少底部大块留白。
- 分类名称在每格中垂直居中，选项之间继续共用一条分隔线；窗口缩放后绘制与点击区域同步调整，不改动导航动作。
- 右侧卡片布局、文字、统一红框及功能流程保持不变；八种终端尺寸下的右侧渲染与 2.2.5 逐格一致。
- 修复残缺鼠标事件或 ESC 残留吞掉下一次点击的问题；解析器从新的事件起点恢复，严格检查数字字段，旧式 X10 坐标按字节读取。
- 保留松开事件不重复触发、拖动与滚轮不触发、缩放时丢弃旧坐标的保护；通过普通与原始模式伪终端中的分段点击、连续操作测试。
- 通过全部 Shell 语法、相关菜单与八种终端尺寸模拟测试，覆盖连续边框、底部出口、页脚防误触和动态缩放；尚未进行掌机实机触控验证，不能据此确认所有偶发漏点均已消除。

## Renkit 2.2.5 紧凑侧栏 — 2026-08-31

- 左侧分类改为共用红色外框，两侧竖线贯通，各选项间仅用一条横线分隔；每项一行名称，窗口变高也不会拉大间距。
- 保留当前分类的亮红提示；分隔线归下一个选项、最后一项包含底边，点击范围互不重叠，菜单下方留白不触发动作。
- 右侧安装、返回与确认按钮的边框统一为柔和红色，危险确认改用亮红文字提示；卡片位置、单列与双列布局及所有功能流程保持不变。
- 通过八种终端尺寸、侧栏边界点击、动态缩放、菜单映射测试及全部 Shell 语法检查；尚未进行掌机实机触控验证。

## Renkit 2.2.4 终端卡片界面 — 2026-08-31

- 首页改为可点击的分类卡片，复用左侧已有分类导航；宽度达到 110 列时，首页及常用软件、插件等纯按钮页面使用双列卡片，窄窗口自动回到单列。
- 侧栏按钮高度统一，普通边框使用柔和暗红，当前分类与危险确认使用亮红；去掉易缺字的侧栏装饰图形，保留完整分类名称。
- 卡片有足够高度时将说明放在名称下方，统一标题、说明与页脚的文字层次；收集完整页面后一次绘制，避免先显示旧排版再跳动。
- 含额外提示或风险说明的页面保留纵向顺序；点击范围按实际卡片位置记录，窗口缩放后重新布局与校准，功能模块、动作 ID、权限确认和安装流程不变。
- 通过八种终端尺寸、双列和单列切换、首页导航、红框边界、中文限宽、风险文字保留及伪终端缩放测试；全部 Shell 语法和相关菜单检查通过。

## Renkit 2.2.3 触控按钮红色边框 — 2026-08-31

- 侧栏九个分类、功能、返回、确认及欢迎页按钮统一使用红色圆角边框；当前分类和危险确认保留加亮提示，不新增背景色块。
- 红框随终端宽高展开，空间充足时文字位于框内；较矮窗口采用名称嵌入上边框的紧凑样式，不占用相邻说明或按钮的位置。24 行窗口优先保留底部按钮完整边框，暂不显示底部操作提示。
- 中文和长说明按可用宽度截断，避免盖住右边框；框内与边框四角都可点击，框外留白不再触发动作。原有安装、返回及安全确认流程不变。
- 八种尺寸的模拟渲染与点击测试覆盖红框完整性、中文限宽、相邻按钮、欢迎页、底部入口以及动态缩放恢复；另通过全部 Shell 语法和菜单映射检查。

## Renkit 2.2.2 终端菜单自适应铺满窗口 — 2026-08-31

- 触控菜单按终端实际宽高布局：侧栏约占四分之一宽度，按钮间距和分隔线随窗口展开，操作提示固定在底部；SteamOS 与 Bazzite 共用该显示层。
- 窗口缩放后自动重绘当前页面，触控区域同步缩放；不重新执行功能模块，不改变菜单动作、权限确认或返回路径。
- 小于 70 列 × 24 行时暂停菜单点击并提示放大；缩放瞬间的旧坐标点击被丢弃，底部提示和分隔区域不会误触功能。
- 新增七种尺寸及伪终端动态缩放模拟测试，覆盖按钮底边、侧栏、返回入口、确认/取消、过小窗口恢复和旧坐标保护。

## Renkit 2.2.1 新版 Clover 开机背景 — 2026-08-30

- “双系统与互通盘 → 应用 Renkit 开机背景”更新为全新的黑红科技风 Renkit 主题，使用 Clover 主题设计基准对应的 1280×800 画布。
- 新背景左侧保持低干扰留白，右侧突出 Renkit 标识；不写死版本号，后续更新无需反复更换背景。
- 应用背景时仍只替换 Clover Apocalypse 主题的 `background.png`，不会改动其他主题文件、EFI 引导文件或启动顺序；模拟测试同步固定新版资源 SHA256。
...
- 修改器所需兼容层由原先四个版本精简为仅安装 GE-Proton 10-29；独立兼容层菜单仍保留其他版本的手动安装入口。
- 机型检测没有匹配到合适的 ROG、联想或 GPD 专用控制插件时，自动安装“掌机功耗控制”SimpleDeckyTDP 中文版；完整安装检测通过时不会重复下载。
- ROG Ally 自动识别补充一代、二代和三代主板标识 `RC71L`、`RC72LA`、`RC73XA`，统一安装 Ally Center；新增模拟测试覆盖系统更新选择、兼容层参数及所有机型插件分支。
...
- 修复 GE-Proton 11-5 下载完成后被误报“压缩包包含预期目录之外的文件”：兼容官方包内 `GE-Proton11-5-x86_64` 顶层目录。
- 安装目录继续规范化为 `GE-Proton11-5`，不会删除已有 GE-Proton 版本；非 x86_64、混合顶层目录、不安全路径和越界符号链接仍会被拒绝。
...
## Renkit 2.0.5 飞行家 F1 BIOS 与 GE-Proton 11-5 — 2026-08-24
...
## Renkit 2.0.4 启动器安装位置选择 — 2026-08-23

- Epic、战网、育碧和黑盒工坊首次准备安装环境前，会在 Konsole 中弹出图形窗口选择内部存储、互通盘、TF 卡或其他 Steam 库。
- 选择窗口显示实际保存路径与剩余空间；启动器虚拟 C 盘、账号数据和默认游戏目录写入所选位置，不会格式化、清空或改动其他磁盘内容。
- 已安装启动器继续复用现有兼容层路径，不重复询问；窗口取消会安全返回，自动化与自定义路径仍可显式指定安装位置。

## Renkit 2.0.3 掌机功耗控制同步上游 1.0.6 — 2026-08-23
...
## Renkit 2.6.4 发布包同步修正版 — 2026-09-12
- 确保发布压缩包同步包含 Ally Center 汉化版本识别修复。
## Renkit 2.6.5 自动更新备用线路修复 — 2026-09-12
- 修复旧版启动时未默认启用 GitHub 更新备用线路的问题，国内镜像短暂超时时会自动继续检测官方备用线路。
## Renkit 2.6.6 Ally Center 原版汉化 — 2026-09-12
- Ally Center 改为严格保留作者原版 v1.2.0 后端、插件清单、配置和控制逻辑；Renkit 仅覆盖中文界面文件。
- 移除 Renkit 的风扇修补功能、扩展风扇选项和界面署名，不再向 Ally Center 注入额外功能。

## Renkit 2.6.7 Ally 控制中心完整包与汉化署名 — 2026-09-13
- 保留 Ally Center 官方 `dist` 目录中的全部文件，仅覆盖中文界面入口；上游新增运行资源不再因汉化安装而丢失。
- Decky 中继续使用稳定的 `Ally Center` 内部服务标识，用户界面保持显示“Ally 控制中心”，避免改名影响插件通信。
- 页面顶端新增“中文汉化：RenAmamiya”署名；补充安装回归测试，校验官方额外 `dist` 文件会被保留。

## Renkit 2.6.8 系统组件更新输出清理 — 2026-09-13
- 过滤 pacman 对已裁剪可选文件输出的“无法获取文件信息”元数据警告，系统组件更新进度不再被大量无害提示刷屏。
- 其余 pacman 输出和真实退出状态均保持不变；更新失败仍会照常停止并显示错误。
