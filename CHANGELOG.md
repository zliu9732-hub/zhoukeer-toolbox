...
- Epic 与育碧服务先尝试无人值守安装，成功后自动创建桌面入口并入库；战网保留官方安装窗口，并在连接更新服务失败后自动切换 Proton 10.0-4 / Proton Experimental 重试。
- Epic、战网和育碧服务新增统一桌面图标，以及 Steam 库横图、竖图、英雄图、图标和徽标；所有位置均带工具箱标识。
- 工具箱菜单、桌面入口和 Steam 库统一使用中文名称“育碧服务”，已安装旧条目会原地更名，避免重复入库。
- 修复旧磁盘 swap 带不可变属性时无法原子替换的问题；替换失败会恢复原文件、激活状态和不可变保护。
- 小黄鸭与 FSR4 插件打开后新增醒目的“闲鱼双叶汉化”署名，同时保留已有汉化说明和原插件作者信息。

## V5.1.0 公开版 — 2026-07-22

- 新增育碧服务官方安装入口，自动检测已安装主程序、创建 Proton 启动包装器、桌面入口并写入 Steam 非 Steam 游戏库。
- 兼容层优先级按启动器区分：仅战网优先 Proton 10.0-4，Epic 与育碧服务优先 Proton Experimental；缺少可用兼容层时仍可通过 Steam 补齐官方 Proton 10。
