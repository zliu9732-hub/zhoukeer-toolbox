## Renkit 2.9.7 帮助页二维码说明排版修复 — 2026-09-23

- 闲鱼说明与二维码说明分别紧跟各自图片绘制，修复右图缩小后二维码提示和网址压到二维码上的问题。
- Steam Deck 800p、1080p 及其他尺寸沿用现有图片大小与左右居中布局。

## Renkit 2.9.6 Steam302 后台初始化自动化 — 2026-09-23

- Steam302 后台服务改为直接运行官方 CLI；检测到旧的 Renkit 托管服务未就绪时自动更新启动项并重启，不再要求手动打开配置入口。
- 完整校验并修复 Steam/GitHub 规则配置，首次初始化最多等待 30 秒；加速未就绪时返回失败并显示日志与依赖排查提示。
- 更新新机交付说明，移除要求手动打开配置界面的过时步骤。

## Renkit 2.9.5 帮助页右图缩小与居中 — 2026-09-23

- 右侧闲鱼图片在所有终端尺寸档统一缩小约 25%，并按实际缩小后的宽高在右栏居中，减少 Steam Deck 800p 和 1080p 边缘裁切。
- 右图在高度上相对二维码居中；账号标题与图片下方说明继续沿用右栏中心。左侧二维码保持原样，2K / 4K 档维持保守尺寸。

## Renkit 2.9.4 F1L 按键修复自动重启 — 2026-09-23

- 特殊按键修复安装或恢复后，显示“操作已完成，5 秒后自动重启 SteamOS；重启后生效，请提前保存工作。”随后自动重启，不再要求输入重启选项。
- 界面确认内容提前说明自动重启时间；增加等待与重启失败提示和日志。
## Renkit 2.9.3 帮助页右图完整显示 — 2026-09-23

- 1080p 的闲鱼图片改用稍窄的完整预览并重新居中，避免右侧裁切；左侧二维码和 Steam Deck 布局不变。
- 未经实机验证的更高分辨率也使用保守的右图宽度，说明仍与二维码底部对齐。

## Renkit 2.9.2 F1L 按键适配与免输入确认 — 2026-09-23

- 掌机适配明确说明：F1L 特殊按键修复仅适用于 F1 8840U，复用普通 F1 映射并补充精确 DMI，其他 F1 型号不纳入白名单。
- 简化掌机适配确认流程：InputPlumber 更新、特殊按键安装与恢复都只需在界面点一次确认，不再要求玩家输入 `UPDATE`、`YES` 或其他文字；绕过菜单直接运行 InputPlumber 更新模块时会安全停止。

## Renkit 2.9.1 高分辨率保守布局 — 2026-09-23

- 2K 与 4K 暂无实机验证，帮助页图片各限制在小一档，优先保证不碰到边界；Steam Deck 和 1080p 布局保持不变。

## Renkit 2.9.0 帮助页右图边缘修正 — 2026-09-23

- 根据 1080p 实拍调整闲鱼图的水平定位，避免右侧内容被终端边缘裁切；二维码和图片尺寸不变。

## Renkit 2.8.9 帮助页二维码居中修正 — 2026-09-23

- 根据 Steam Deck 实拍修正左侧二维码的水平居中位置；保持图片尺寸和右侧闲鱼图位置不变。

## Renkit 2.8.8 Steam Deck 帮助图片尺寸修正 — 2026-09-23

- 根据 Steam Deck 800p 实拍修正终端字号到图片空间的换算，避免矮窗口误选最小的 123px 二维码。
- 增加 246px 二维码及配套闲鱼图档位；更宽的 800p 窗口可继续选择更大图片，同时保留说明文字和返回按钮空间。

## Renkit 2.8.7 帮助图片填充优化 — 2026-09-23

- 帮助页按终端可用宽高和字号自动选择图片尺寸：Steam Deck 800p 保持紧凑，1080p、2K、4K 逐级放大二维码与闲鱼图，减少页面空白。
- 图片底部预留说明文字和返回按钮空间；缩小窗口时自动回退到较小图片。

## Renkit 2.8.6 帮助说明对齐修正 — 2026-09-22

- 扫码失败提示改为两行并完整收回左侧二维码下方，网址紧跟其后，不再横跨两张图片中间。
- 右侧“闲鱼关注我了解更多资讯”保持在闲鱼图片正下方。

## Renkit 2.8.5 帮助主菜单醒目显示 — 2026-09-22

- 左侧“帮助”改为更亮的粗体红字并增加全角字距，视觉尺寸比其他主菜单更大，不改变边框、行高或触控区域。

## Renkit 2.8.4 帮助图片自适应放大 — 2026-09-22

- 帮助页图片根据终端宽度和高度自动选择小、中、大三档，常见与高分辨率窗口使用更大图片，小窗口自动回退以避免重叠。
- 图片下方文字改为跟随图片实际底边紧凑排版，不再使用固定行号造成大段留白。
- 二维码网址上方新增“如果二维码扫不出来请在浏览器浏览以下网址”提示。

## Renkit 2.8.3 帮助图片内嵌显示 — 2026-09-22

- 点击左侧“帮助”后，联系二维码与闲鱼主页图现在直接显示在 Renkit 右侧内容区，不再要求点击按钮打开外部页面。
- 两张图片按参考样式左右并排；顶部显示社区支持标语，右侧显示“闲鱼账号：RenAmamiya”。
- `https://link3.cc/renamamiya` 仅作为一行网址放在二维码下方，不再显示“联系网址”前缀。

## Renkit 2.8.2 帮助页排版与菜单位置 — 2026-09-22

- 帮助主菜单移到“免责声明与须知”和“退出Renkit”之间，并使用红字醒目显示。
- 帮助页改为简洁的左右并排布局：联系二维码在左，闲鱼账号 `RenAmamiya` 与主页图在右。
- 二维码底下保留完整链接 `https://link3.cc/renamamiya`，闲鱼图片底下显示“闲鱼关注我了解更多资讯”。

## Renkit 2.8.1 启动更新超时保护 — 2026-09-22

- 启动自动更新增加 60 秒硬性超时；网络或镜像异常时会停止本次更新并直接进入现有版本，不再长期卡在“Renkit启动中”。
- 手动更新仍保留完整下载、校验和多源回退流程，启动超时不会覆盖旧版本或留下半更新目录。

## Renkit 2.8.0 帮助与联系入口 — 2026-09-22

- SteamOS 左侧主菜单新增独立“帮助”入口，与“游戏与插件”“检查与维护”等主分类同级。
- 帮助页面将联系二维码与闲鱼主页图片左右并排展示，并在图片下方显示完整网址和关注提示；窄屏自动改为上下排列。

## Renkit 2.7.9 小黄鸭 2.0 使用提示 — 2026-09-20

- 小黄鸭 2.0 入口明确提示：仅供正版小黄鸭用户使用、替代 1.0 小黄鸭，并可与 MAKO 共存。

## Renkit 2.7.8 小黄鸭 2.0 入口文案 — 2026-09-20

- 小黄鸭 2.0 的触控和图形菜单入口不再展示“置顶 RenAmamiya 汉化署名”；保留版本、完整包校验和与 1.0、MAKO 共存的说明。

## Renkit 2.7.7 下载进度与桌面快捷方式 — 2026-09-20

- 修复触控主界面的下载进度被启动日志过滤器积压的问题：现在下载过程中会实时刷新单行进度条、百分比和速度。
- 已安装软件在桌面已有同一应用的有效快捷方式时，保留用户的原入口，不再创建 Renkit 的重复 `.desktop` 文件。

## Renkit 2.7.3 pacman 原生下载进度 — 2026-09-15

- 系统组件更新在交互式终端直接保留 pacman 原生下载进度，逐包显示准确百分比与实时速度，不再只显示“正在获取软件包”。
- 非交互日志和自动测试仍过滤 SteamOS 已裁剪可选文件产生的无害元数据警告，真实错误和退出状态不受影响。

## Renkit 2.7.2 OneXPlayer Apex 汉化组件交付修复 — 2026-09-14

- 修复安装器遗漏 OneXPlayer Apex Tools 汉化前端的交付问题；现在安装时会一并部署已有的完整中文组件，不再误报“中文组件不完整”。
- 保留上游原版后端和既有汉化前端；插件页面顶端继续显示“中文汉化：RenAmamiya”。
- 新增发布白名单回归检查，防止后续打包再次遗漏该掌机插件的中文组件。
...
- InputPlumber 0.79.2、飞行家 F1 V1.14 BIOS、X2 Mini Pro 三份固定配置和 RustDesk AppImage 改为 国内镜像 分块镜像优先，官方 GitHub 仅作失败回退。
- 国内镜像继续校验固定 SHA256、文件大小和分块清单；任何校验失败都会保留原系统或已安装版本。
- 新增独立自动同步模式，代码更新后先把固定资产上传至 国内镜像，再发布 Renkit 客户端更新。
...
## Renkit 2.7.4 Ally 控制中心完整汉化组件 — 2026-09-16

- 修复发布安装包遗漏 Ally Center 中文插件清单的问题；安装后 Decky 正确显示“Ally 控制中心”及 RenAmamiya 汉化署名。
- 保留作者 v1.2.0 的后端、运行时资源与版本，仅原子覆盖已校验的中文前端和插件清单；缺少任一中文组件会停止覆盖，避免半安装状态。
## Renkit 2.7.5 小黄鸭 2.0 — 2026-09-20

- 小黄鸭版本选择新增 v0.14.4 的“小黄鸭 2.0”；小黄鸭 1.0、2.0 与 MAKO 可同时保留，MAKO 原有流程与署名不变。
- SteamOS、Bazzite 和 ChimeraOS 均已提供 1.0、2.0、MAKO 三个分支；2.0 通过国内分块镜像下载、校验完整 ZIP、目录、许可证、运行文件和前端 SHA256。
- 2.0 完整包保留原作者与 BSD-3-Clause 许可证，并在插件内容最上方加入与 1.0 一致的黄色“RenAmamiya汉化”置顶署名；未修改来源仓库或其 Release。

## Renkit 2.7.6 小黄鸭 2.0 镜像迁移 — 2026-09-20

- 小黄鸭 2.0 的专用国内镜像已从 mirror-3 迁至独立的 mirror-4；小黄鸭 1.0 与 MAKO 继续使用原有镜像，不受影响。
- 安装器固定校验原有完整包 SHA256，镜像迁移不改变小号公开 Release、插件内容、署名或许可证。
## Renkit 2.9.8 InputPlumber 通用软件包更新 — 2026-09-26

- 启动 Renkit 或执行系统更新时，检查已安装且正在使用的 InputPlumber，通过系统软件源比较并升级；已是最新版本时跳过。
- 移除更新入口中的壹号掌机 DMI 限制、固定版本下载和直接写入 `/usr` 的方式。机型按键配置仍由独立模块管理。
- 升级后重启并检查服务；失败时记录错误与最近服务日志，不阻断 Renkit 其他功能。SteamOS 存在其他待更新包时跳过单包升级，避免部分升级。
## Renkit 2.9.9 修正中文系统下的 InputPlumber 软件源版本识别 — 2026-09-26

- 固定 pacman 软件源查询的输出语言，避免中文字段名被误判为“软件源没有 InputPlumber 包”。
- 软件源查询失败或版本无法解析时显示已安装版本，并明确提示本次未更新；继续保持系统软件包管理方式及安全跳过规则。
