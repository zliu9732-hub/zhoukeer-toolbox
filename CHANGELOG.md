...
## V1.6.4 恢复双系统互通盘与 Clover 完整安装/修复 — 2026-08-07

- 恢复双系统互通盘、TF 卡、磁盘写入修复、互通盘保护、双系统引导修复和一键切换 Windows 菜单。
- 修复双系统引导改为完整 Clover 安装/修复：按设备复制 cloverx64.efi、设备配置、主题和 ROG 驱动，并启用 Clover 开机修复服务。
- Clover 资源改为自有 Gitee 分块镜像下载，不再走国外更新通道。
- TF 卡初始化改为 NTFS，支持自定义卷标；磁盘写入修复会清理失效 Steam 下载/着色器软链接。
- 一键切换 Windows 使用 BootNext 并立即重启，执行前需要明确确认。

## V1.6.3 封面素材改为 Gitee 分块镜像静默下载并加固 Steam 写入流程 — 2026-08-07

- 启动器封面素材不再打进工具箱发布包，改为从 Gitee 镜像仓库分块下载整个封面包，校验 SHA256 后缓存到本地；下载过程不向用户显示进度。
- 封面主图、竖版图、横幅和背景统一改用压缩 JPG，徽标与图标保留 PNG，封面包体积明显缩小。
- 封面写入失败不再中断安装，Steam 条目和兼容层会继续完成；安全停止 Steam 后即使后续步骤失败也会重新启动 Steam。
- Steam 未初始化或未登录时输出详细提示，明确说明需要先打开 Steam、完成登录并等待库初始化后再运行。
- 删除已不使用的欢迎图和旧 Windows 切换图标。

## V1.6.2 插件更新检测、卸载清理与不依赖 Decky 的封面修复 — 2026-08-07

- CheatDeck 现在按版本号检测旧版本并自动更新，状态页同步显示官方版本，不再只凭目录存在跳过。
- 卸载 Decky Loader 后顺带清理残留空目录，插件目录和设置按设计保留。
- 新增“重新应用启动器封面”入口（触控、GUI 和 CLI），重写 Steam 库封面并重启 Steam，不依赖 Decky。
- 启动器安装完成后不再等待 Steam 完全重启或 Decky 刷新，网络慢时不会卡在安装窗口。
...
- 支持安装状态检测、桌面快捷方式、卸载菜单和桌面图标修复。

## V1.4.0 全部启动器统一可见路径并统一 Proton 10.0-4 兼容层 — 2026-08-06

- Epic、育碧、战网、黑盒工坊全部改为直接使用 `~/游戏启动器` 可见路径写入 Steam 库，不再使用旧隐藏前缀路径。
- 四个启动器统一写入 Proton 10.0-4 兼容层映射，并同时写入常规与备用两种 Steam AppID。
- 安装完成后自动启动 Steam，并尝试通过 Decky 即时刷新封面。
...
## V1.3.8 启动器安装后自动启动 Steam 并即时刷新封面 — 2026-08-06

- 战网、Epic、育碧、黑盒工坊安装完成或正式入库后自动启动 Steam，不再要求用户手动打开。
- 移除“请手动启动 Steam”等安装描述，改为提示 Steam 已自动启动。
- Steam 启动后自动尝试通过 Decky 即时刷新封面，并确认 Proton 10.0-4 兼容层。

## V1.3.7 战网 Steam 条目改用可见路径并重写封面兼容层 — 2026-08-06

- 战网 Steam 库条目改为直接指向 `~/游戏启动器/battlenet/drive_c/...`，不再显示旧隐藏前缀路径。
- 再次安装战网时会按新路径重新计算 Steam appid、写入封面并绑定 Proton 10.0-4。
- 迁移时新位置存在空目录也会自动接管，不再因空目录中断。
- GE-Proton 下载和解压临时目录改到兼容层所在磁盘，避免 Steam Deck 的 `/tmp` tmpfs 空间不足导致“设备上没有空间”。

## V1.3.6 迁移到新 Gitee 仓库并默认只走 Gitee — 2026-08-06

- 工具箱安装与更新源迁移到 `zhoukeer-toolbox-v2`，默认只从 Gitee 下载，不再默认回退 GitHub 或域名源。
- Decky 镜像、独立工具引导和网络诊断同步切换到新仓库地址。
- 第三方插件、GE-Proton 等仍保留各自作者来源，不受本次迁移影响。

## V1.3.5 修改器镜像自动重试与旧隐藏目录迁移 — 2026-08-06

- 修改器常用兼容层镜像下载增加自动重试：单次失败最多自动重试 3 次，仍只使用 Gitee 自有镜像，不切换 GitHub。
- 旧版安装在隐藏目录中的启动器，再次点击安装入口时会自动把 Windows 虚拟目录迁移到用户可见的 `~/游戏启动器`，并重写 Steam 条目与 compatdata drive_c 链接。

## V1.3.4 启动器虚拟目录移至用户可见根目录并直接入库 — 2026-08-06

- 战网、Epic、育碧等启动器的 Windows 虚拟 C 盘默认放到 `~/游戏启动器/<启动器>/drive_c`，不再放在隐藏的 `~/.local/share` 目录，黑盒工坊等插件可以直接找到游戏文件；可用 `ZHOUKEER_LAUNCHER_BASE` 自定义位置。
- Epic 与育碧改为把真实 Windows EXE 直接写入 Steam 库，自动绑定 Proton 10.0-4，并把 Steam compatdata 的 drive_c 链接到同一份可见目录。
- Steam 条目图标改为使用 grid 封面目录中的图标文件，和库封面保持一致。
- 保留桌面独立启动包装器；已有安装再次点击安装入口即可自动重写 Steam 条目与链接。

## V1.3.3 修复育碧等启动器安装后打不开 — 2026-08-06

- 修复启动包装器把带空格的 Proton 与主程序路径错误写成反斜杠转义的问题；育碧、Epic、战网和黑盒工坊重新安装后会自动生成可用的启动脚本。
- 已安装用户再次点击对应启动器安装入口即可重写桌面启动包装器，无需卸载重装。

## V1.3.2 战网与黑盒工坊安装后提示 Proton 兼容层 — 2026-08-06

- 战网启动器与黑盒工坊安装完成后，醒目提示在 Steam 库点击游戏右侧齿轮 → 属性 → 兼容性，勾选“强制使用兼容性工具”并选择 Proton 10.0-4。
- GUI 模式安装成功后会再次弹出该提示。

## V1.3.1 GE 兼容层自动重启 Steam 并新增 LICENSE — 2026-08-06

- GE 兼容层（最新版与修改器常用兼容层）安装完成后自动重启 Steam，不再要求用户手动重启。
- 已安装的兼容层会自动跳过，不会重复下载安装。
- 新增 LICENSE：个人免费使用，禁止商业使用、销售、转卖或二次盈利发布。

## V1.3.0 修复 Steamcommunity 302 后台加速未生效 — 2026-08-05

- 后台服务与内置启动统一使用官方 `steamcommunity_302.cli Service` 参数启动，修复安装成功但加速不生效的问题。
- 不再覆盖官方 `S302.ini` 的 `[Setting]` 开关，确保 Steam 商店与 GitHub 规则保持启用。
- Steamcommunity 302 子菜单新增“打开官方配置界面”，便于完成首次配置。
- 安装包继续从自有 Gitee 镜像下载。
- 常用软件新增“中文输入法”：通过 Flathub 国内缓存安装 Fcitx5 与中文输入插件。
- GE 兼容层入口改为子菜单：安装最新版时不再删除旧版兼容层。
- 新增“安装修改器所需常用兼容层”：安装 GE-Proton 7-55、8-25、9-27、10-29，合计约 1.72GB，下载较慢为正常现象。
- 下载 CheatDeck 或安装常用插件组合时，提示进入“游戏与插件”安装修改器所需兼容层。

## V1.2.9 优化 Steamcommunity 302 加速 — 2026-08-05

- Steamcommunity 302 安装与更新只从自有 Gitee 镜像下载，不再连接其他来源。
- “一键开启加速”优先启用并启动工具箱托管的 systemd 后台服务，新增“重置加速”入口。
- 撤销开启 302 加速后自动启动 Steam 的行为，只保留自动开启加速。
- 同步 FSR4 汉化完整包 v0.17.0 版本号与校验值。

## V1.2.8 开启 302 加速后自动启动 Steam — 2026-08-05

- Steamcommunity 302 菜单“一键开启加速”在加速服务启动后自动拉起 Steam，不再需要手动找入口。
- 保留下载较慢时自动启用 302 加速重试的原有逻辑。
...
## V1.2.4 修复 FSR4 汉化完整包 — 2026-08-04

- FSR4 版本号统一为 0.17.0，与作者 Release 命名保持一致。
- 修复汉化完整包打包错误导致安装后界面为英文的问题；镜像与 Release 资产均已替换为正确汉化包。
- 未署名版本不再携带作者署名。

## V1.2.3 FSR4 汉化下载优化 — 2026-08-04

- FSR4 汉化完整包改为优先从新 Gitee 镜像仓库下载，国内更快更稳；失败自动回退 GitHub 原版叠加。
- 镜像下载过程不再显示“分块/重组”等细节，只保留简洁进度提示。

## V1.2.2 FSR4 汉化同步 v0.17 — 2026-08-04

- FSR4 插件同步官方 v0.17：新增 4.1.1 驱动覆盖与 Valve RDNA2 兼容运行库，前端中文界面与汉化署名同步更新。
- 新增“战网 + 黑盒工坊”独立安装工具与一行安装引导，可从 Gitee 单独下载使用。

## V1.2.1 版本号更新 — 2026-08-04

- 内容与 V1.2.0 相同，仅更新版本号，确保已安装 V1.2.0 的设备能自动更新到包含修复的新包。

## V1.2.0 优化启动器与自动更新体验 — 2026-08-04

- 优化黑盒工坊与战网安装体验。
- 优化兼容层写入时机，安装后自动绑定 Proton 10.0-4。
- 修复启动器 Steam 库背景图成套落地。
- 修复 Epic、战网、育碧、黑盒工坊 Steam 条目残留启动项与 StartDir 内层双引号，桌面入口不受影响，便于叠加小黄鸭 `~/lsfg %command%`。
- 修复自动更新版本检测：同时检查国内镜像、GitHub 和域名源并取最高版本，域名源单独可达且版本未更新时不再误报“已是最新”。
- 补齐黑盒工坊桌面图标素材，安装后桌面与 Steam 库显示正式图标。

## V1.1.14 战网子菜单与黑盒工坊安装 — 2026-08-04

- 战网入口改为子菜单，新增“黑盒工坊”安装项；安装流程与育碧/Epic 一致，从 Gitee 分块镜像下载 `wow_installer_1.9.51.0.exe`，通过 Proton 静默/可见窗口安装并自动入库。
- 黑盒工坊安装包 SHA256 固定为 `9e0bce560d8264eb015a020337167f57918babd755d1671c38a49f3cdb05654a`，首次使用占位封面资源，后续可替换为官方素材。
