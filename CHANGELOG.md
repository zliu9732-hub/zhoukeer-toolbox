## V1.1.15 战网与黑盒工坊安装改进 — 2026-08-04

- 战网安装改为预装客户端流程：自动下载并校验客户端分卷，重组解压到独立 C 盘环境，绑定 Proton 10.0-4 后自动覆盖 Steam 条目；下载失败才回退到 Steam 库手动安装。
- 黑盒工坊改为预装客户端一键安装：客户端来自工具箱 Gitee 镜像，自动校验解压到战网同一 C 盘环境，绑定 Proton 10.0-4 后自动写入 Steam 条目；下载失败才回退到 Steam 库手动安装。
- 修复启动器 Steam 库背景图丢失：史诗、战网、育碧恢复写入 `_background.jpg`，黑盒工坊写入 `_background.png`，主页格图、竖版封面、hero、logo、图标与背景全部成套落地。
