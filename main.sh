...
show_disclaimer() {
    ...
    while true; do
        ...
        ui_disclaimer_line 8 '\033[1;38;5;220m' "RenAmamiya制作"
        ui_disclaimer_line 9 '\033[38;5;45m' "创作者：RenAmamiya"
...
lsfg_versions_menu() {
    ...
    while true; do
        ...
        ui_touch_button 22 '\033[1;97;48;5;238m' "返回首页" "查看全部功能分类"
        ui_prompt
        choice="$(read_touch_menu right:5-6:stable right:8-9:v2 right:11-12:mako right:19-20:back right:22-23:home)"
        if apply_navigation "$choice"; then return 0; fi
        case "$choice" in
            stable)
    ...
    while true; do
        ...
        case "$choice" in
            ...
            test)
                ...
                ;;
            rog-white-install)
                confirm_and_run "安装 ROG White 白色主题" "将 Renkit 内置的 ROG White v1.4.9 白色主题放入 CSS Loader 主题目录；安装后请在 CSS Loader 中开启该主题" \
                    env ZHOUKEER_AUTO_CONFIRM=1 bash "$PROJECT_ROOT/modules/rog_white_theme.sh" install
                ;;
            handheld-pink-install)
                confirm_and_run "安装 掌机 Pink 粉色主题" "将 Renkit 内置的 Handheld Pink v1.0.3 粉色主题放入 CSS Loader 主题目录；安装后请在 CSS Loader 中开启该主题" \
                    env ZHOUKEER_AUTO_CONFIRM=1 bash "$PROJECT_ROOT/modules/handheld_pink_theme.sh" install
                ;;
            pink-white-gradient-install)
                confirm_and_run "安装 粉白渐变 粉色主题" "将 Renkit 内置的 Pink White Gradient v1.0.2 浅粉渐变主题放入 CSS Loader 主题目录；安装后请在 CSS Loader 中开启该主题" \
...
plugin_page_2_menu() {
    ...
    while true; do
        ...
        ui_touch_button 23 '\033[1;97;48;5;238m' "返回首页" "查看全部功能分类"
        ui_prompt
        choice="$(read_touch_menu right:5-6:deckrecall right:7-8:savepulse right:9-10:handheld-plugins right:11-12:unifideck right:13-14:tomoon right:15-16:ge-proton right:17-18:launchers right:19-20:game-info-tools right:21-22:previous right:23-24:home)"
        if apply_navigation "$choice"; then return 0; fi
...
game_info_plugins_menu() {
    ...
    while true; do
        ...
        [ "$NEXT_CATEGORY" = "game_info_plugins" ] || return 0
    done
}
    ...
    while true; do
        ...
        case "$choice" in
            ...
            uninstall)
                ...
                ;;
            button-install)
                confirm_and_run "壹号掌机 SteamOS 特殊按键修复" "仅支持界面列出的实机验证型号；将写入对应配置、备份同名原文件。操作完成后等待5秒自动重启SteamOS，请提前保存工作" \
            ...
            button-status)
                ...
                ;;
            button-restore)
                confirm_and_run "恢复壹号掌机 SteamOS 特殊按键修复" "将校验并恢复原配置，同时还原 InputPlumber 修复前的启用与运行状态。完成后等待5秒自动重启SteamOS，请提前保存工作" \
...
help_contact_menu() {
    ...
    while true; do
        ...
        ui_help_image 6 right "$PROJECT_ROOT/assets/help/xianyu-renamamiya.sixel"
        ui_help_right_caption "闲鱼关注我了解更多资讯"
        ui_help_image 6 left "$PROJECT_ROOT/assets/help/renamamiya-qr.sixel"
        ui_help_captions "如果二维码扫不出来" "请在浏览器浏览以下网址" \
            "https://link3.cc/renamamiya"
        ui_touch_button 22 '\033[1;97;48;5;238m' "返回首页" "查看全部功能分类"
        ui_prompt
