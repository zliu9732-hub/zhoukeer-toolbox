...
const manifest = {"name":"Decky-Framegen(FSR4)","author":"Kurt Himebauch","flags":[],"api_version":1,"publish":{"tags":["DLSS","Framegen","upscaling","FSR"],"description":"汉化：Ren-Amamiya-pixle / zliu9732-hub（闲鱼RenAmamiya）。管理 OptiScaler，为 DirectX 12 游戏提供超分辨率与插帧支持。","image":"https://raw.githubusercontent.com/xXJSONDeruloXx/Decky-Framegen/refs/heads/main/assets/optiscaler_final.png"}};
const API_VERSION = 2;
...
const ManualPatchControls = ({ isAvailable, onManualModeChange, dllName, fsr4Variant }) => {
    ...
    return (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
        ...
        canInteract && (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
            ...
            window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.ButtonItem, { layout: "below", onClick: openDirectoryPicker, description: "\u9009\u62E9\u6E38\u620F\u5B89\u88C5\u76EE\u5F55\uFF08EXE \u6587\u4EF6\u6240\u5728\u4F4D\u7F6E\uFF09\u3002" }, "\u9009\u62E9\u76EE\u5F55")),
...
function SteamGamePatcher({ dllName, fsr4Variant }) {
    ...
    const loadGames = SP_REACT.useCallback(async () => {
        ...
        try {
            ...
            if (result.status !== "success")
                throw new Error(result.message || "读取游戏列表失败。");
    ...
    const selectedVariantLabel = SP_REACT.useMemo(() => FSR4_VARIANT_OPTIONS.find((option) => option.value === fsr4Variant)?.label ?? fsr4Variant, [fsr4Variant]);
    ...
    const handlePatch = SP_REACT.useCallback(async () => {
        ...
        try {
            ...
            if (result.status !== "success")
                throw new Error(result.message || "修补失败。");
    ...
    const handleUnpatch = SP_REACT.useCallback(async () => {
        ...
        try {
            ...
            if (result.status !== "success")
                throw new Error(result.message || "撤销修补失败。");
            setAppLaunchOptions(Number(selectedAppId), result.launch_options || "");
    ...
    const statusDisplay = SP_REACT.useMemo(() => {
        ...
        if (statusLoading)
            return { text: "正在读取…", color: undefined };
        if (!gameStatus || gameStatus.status === "error")
            return { text: gameStatus?.message || "—", color: undefined };
        if (!gameStatus.install_found)
            return { text: "未找到游戏安装目录", color: "#ffd866" };
        if (!gameStatus.patched)
            return { text: "未修补", color: undefined };
    ...
    return (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
            window.SP_REACT.createElement(DFL.DropdownItem, { layout: "below", label: "Steam \u6E38\u620F", menuLabel: "\u9009\u62E9 Steam \u6E38\u620F", strDefaultLabel: gamesLoading ? "正在读取游戏列表…" : "选择游戏", disabled: gamesLoading || games.length === 0, selectedOption: selectedAppId, rgOptions: games.map((g) => ({
                    data: g.appid,
        ...
        selectedGame && (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
            window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.Field, { ...focusableFieldProps, label: "\u4FEE\u8865\u72B6\u6001" }, statusDisplay.color ? (window.SP_REACT.createElement("span", { style: { color: statusDisplay.color, fontWeight: 600 } }, statusDisplay.text)) : (statusDisplay.text))),
            window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.Field, { ...focusableFieldProps, label: "FSR4 \u8FD0\u884C\u5E93" }, gameStatus?.patched
            ...
            canUnpatch && (window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.ButtonItem, { layout: "below", disabled: busyAction !== null, onClick: handleUnpatch }, busyAction === "unpatch" ? "正在撤销…" : "撤销此游戏修补"))),
            window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.ButtonItem, { layout: "below", disabled: !selectedAppId || busyAction !== null || statusLoading, onClick: () => void loadStatus(selectedAppId) }, statusLoading ? "正在刷新…" : "刷新状态")),
            resultMessage && (window.SP_REACT.createElement(DFL.PanelSectionRow, null,
                window.SP_REACT.createElement(DFL.Field, { ...focusableFieldProps, label: "\u7ED3\u679C" }, resultMessage)))))));
}
...
function OptiScalerControls({ pathExists, setPathExists, fgmodInfo }) {
    ...
    return (window.SP_REACT.createElement(DFL.PanelSection, null,
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
            window.SP_REACT.createElement(DFL.Field, { label: "Ren-Amamiya-pixle / zliu9732-hub（闲鱼RenAmamiya）\u6C49\u5316", description: "FSR4 \u4E2D\u6587\u754C\u9762" }, "\u539F\u63D2\u4EF6\u4F5C\u8005\uFF1AKurt Himebauch\uFF08xXJSONDeruloXx\uFF09")),
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
