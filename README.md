...
5. 只显式暂存本次发布文件，提交代码并打 tag，例如 `v1.1.6`；禁止 `git add .`、force push 或改写历史。
...
当前正式版为 V1.1.6，版本号按语义化版本递增，不跳版。后续维护继续围绕 SteamOS 实际使用更新下载链路、插件镜像、菜单与修复；rEFInd、Clover、EFI/BootOrder 和“一键切换 Windows”等高风险入口保持停用。
