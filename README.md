...
远程协助中提供 RustDesk、AnyDesk 和 ToDesk。RustDesk 使用 Gitee 分块镜像优先、作者 GitHub Release 回退的官方 AppImage，并自动创建桌面图标，不会修改 SteamOS 只读系统分区，也不会被Renkit自动写入任何服务器配置；AnyDesk 通过 Flathub 国内镜像安装；ToDesk 使用固定官方 DEB 的 SteamOS 适配包。
