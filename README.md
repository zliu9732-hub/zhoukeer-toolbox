...
当前正式版：Renkit 2.7.3；从 Renkit 1.0 起按语义化版本递增。
...
远程协助中提供 RustDesk、AnyDesk 和 ToDesk。RustDesk 使用国内分块镜像优先、作者 GitHub Release 回退的官方 AppImage，并自动创建桌面图标，不会修改 SteamOS 只读系统分区，也不会被Renkit自动写入任何服务器配置；AnyDesk 通过 Flathub 国内镜像安装；ToDesk 使用固定官方 DEB 的 SteamOS 适配包。
...
- 插件商城：SteamOS 的九款常用组合包含小黄鸭 1.0、FSR4、CheatDeck、MAKO 小黄鸭与 `Mako_Renkit` 启动项、游戏封面更换（SteamGridDB）、主题美化（CSS Loader 中文版）、文件传输助手（Friendeck）、音乐播放器（Decky Music）和 Fantastic 风扇控制汉化版；小黄鸭 2.0 作为独立版本在三个系统的版本选择菜单中提供。小黄鸭 1.0、2.0 与 MAKO 会同时保留，用户可自行选择；2.0 使用完整 v0.14.4 中文包，保留原作者和 BSD-3-Clause 许可证，并在顶部显示“RenAmamiya汉化”黄色署名。Fantastic 同时列在“其余常用插件”，由 Renkit 从国内镜像直接下载、校验并自动安装，不调用 Decky 商城弹窗。另提供 Decky 官方精选插件；支持整组、单项安装和状态检查。Decky Music 使用作者原版 v1.0.2 完整包，内置播放器与 QQ/网易云音乐源，不汉化、不重打。Decky Loader 子菜单提供 ROG White 白色主题一键安装，把主题文件放入 CSS Loader themes 目录后即可在 CSS Loader 中开启。独立提供 DeckRecall 与 SavePulse：SavePulse 可自动保留存档版本，并使用每位用户自己的坚果云或标准 WebDAV 做加密备份和换机恢复。固定版本均执行 SHA256、ZIP 结构与插件目录校验。安装或检测到功能插件后会在桌面补充风灵月影、小黄鸭和 FSR4 小白教程；检测到 FSR4 时另建 OptiScaler 官方 Wiki 已测试游戏清单。小黄鸭安装完成后会自动检测 Steam 库中是否已有 Lossless Scaling：已安装会提示可继续使用，未安装会打开 Steam 正版页面。
...
- ROG Ally适配：掌机控制插件与新机初始化可安装 Ally Center v1.2.0 中文版。Renkit 保留作者原版后端、插件清单、配置和内部服务标识，只覆盖中文界面入口并保留官方 `dist` 中的其余运行文件；Decky 中显示“Ally 控制中心”，页面顶端标注“中文汉化：RenAmamiya”。
- 壹号掌机适配：屏幕方向修复支持 `ONEXPLAYER F1` 7840U、8840U OLED 实机 DMI 名 `ONEXPLAYER F1L`，并保留 `ONEXPLAYER F1 OLED` 兼容名称。特殊按键修复中的 F1L 支持**仅适用于 F1 8840U（精确 DMI：`ONEXPLAYER F1L`）**；因 F1L 与普通 F1 使用同一套映射，Renkit 复用 SteamOS 自带的 `50-onexplayer_onexfly.yaml`，只在 `/etc/inputplumber/devices.d/50-onexplayer_f1l.yaml` 副本中把首个普通 F1 型号改为 F1L，不修改系统源文件。特殊按键修复另支持实机验证过的 `ONEXPLAYER X1Pro`；`ONEXPLAYER X2Mini PRO` 使用独立的固定提交配置，补齐设备 DMI、`QuickAccess` 映射与 SteamOS 虚拟设备目标，修复机身 OXP 键的三点菜单。三机型均可查看状态和恢复原文件；X2 Mini Pro 需临时关闭只读保护以写入 steamos-manager 配置，完成后自动恢复。所有按键修复均不安装 HHD，且检测到 HHD 运行时会停止。另提供壹号掌机专用 InputPlumber 0.79.2 官方固定包更新，使用国内分块镜像优先、固定 SHA256、GitHub 官方回退、包内路径/版本检查和旧版备份；该高级操作会临时关闭只读保护并写入 `/usr`，完成后恢复原只读状态，且可能被 SteamOS 大版本更新覆盖。X2 Mini Pro 三份固定配置与 V1.14 BIOS 文件也优先使用国内镜像；BIOS 入口仍仅适用于 ONEXPLAYER F1 / ONEXFLY 7840U 普通黑白版，Renkit 只校验并复制原厂文件，不会在 SteamOS 下刷写 BIOS。
- 壹号掌机特殊按键修复：安装或恢复并通过配置验证后，工具箱提示 5 秒后自动重启 SteamOS；请提前保存工作，重启后生效。
- ToDesk：使用固定的第三方SteamOS适配包并校验SHA256，安装完成后恢复只读保护。
- Steam Deck 优化：清理 Steam 下载缓存、着色器缓存，并提供性能模式提示。
- 国内下载源与系统组件：完整同步系统组件，重装匹配的 Discover、PackageKit 与 Qt6 运行库，并自动恢复曾被停用的 PackageKit 后端；archlinuxcn 使用上海交大、中科大和官方 HTTPS 镜像逐级回退，安装并加载 GPG 密钥环。完成 Flatpak 国内缓存配置后，会限时修复用户仓库、刷新 AppStream、重建 KDE 缓存并重新启动 Discover；新机初始化会把这部分转入后台，不阻塞后续软件与插件安装。pacman 对系统已裁剪可选文件产生的“无法获取文件信息”元数据警告会被过滤，真实错误和退出状态仍完整保留；任何失败都会恢复 SteamOS 原有只读保护状态。
...
- 自动更新Renkit：每次启动会快速检测国内镜像、GitHub 与备用域名的版本，以可达来源中的最高正式版为准，并从对应来源优先下载和校验更新；断网或更新失败时继续启动现有版本。
- 连续下载进度：软件、插件、启动器、国内镜像和 Renkit 更新包统一显示单行 20 格进度条，格式为“百分比（实时速度）”；HTTP 跳转产生的伪完成帧会被丢弃，百分比只增不减，并在最后一帧输出完成后才进入下一项，避免进度条上下重叠。
...
```bash
curl -fsSL https://raw.githubusercontent.com/zliu9732-hub/zhoukeer-toolbox/main/i | bash
```
...
curl -fsSL https://raw.githubusercontent.com/zliu9732-hub/zhoukeer-toolbox/main/scripts/install-decky-plugin.sh | bash -s -- framegen
curl -fsSL https://raw.githubusercontent.com/zliu9732-hub/zhoukeer-toolbox/main/scripts/install-decky-plugin.sh | bash -s -- cheatdeck
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/zliu9732-hub/zhoukeer-toolbox/main/bootstrap.sh | bash -s -- --dry-run
```
...
Renkit会生成并校验完整的 Steam 与 GitHub 规则配置，直接运行官方 CLI 后台程序，并创建带管理标记的 systemd 服务，安装完成后立即启动并设置开机自启。证书、hosts、DNS 和本地代理初始化由官方 CLI 在后台完成，无需先打开图形配置入口。启动、更新、重置和状态检查会验证官方 `S302.run` 就绪标记，或本地 DNS 53 与代理 80/443 监听；后台进程运行但代理未就绪时会报告日志、端口占用或系统依赖检查方式。桌面不会出现 Steamcommunity 302 图标；停止或卸载时只处理Renkit生成的同名服务，不覆盖或删除其他程序创建的服务。官方 CLI 仍可能按自身实现修改 hosts、DNS、证书或本机代理，使用前请阅读风险说明。
...
3. 更新 `VERSION` 与 `CHANGELOG.md`，确认本机 SSH 可以推送生产国内更新仓库。
...
6. 只推送 `main` 与当前版本 tag（例如 `v1.6.1`），禁止使用 `git push --tags`；避免把已清理的旧标签带回仓库。再向生产国内更新仓库同步版本化发布包和 `.sha256` 校验文件。
7. 在 Release 中写明安装、更新、卸载命令。
8. 不要在 Release 包中包含密码、Token、邮箱或个人路径；桌面的 `管理员密码.txt` 仅在用户设备本地生成。

默认下载顺序：

1. 国内更新仓库固定包：`dist/renkit.tar.gz`

## 当前版本与维护

当前正式版为 Renkit 2.7.1，后续版本从 1.0 起按语义化版本递增。后续维护覆盖 SteamOS、Bazzite 与 ChimeraOS 的独立菜单；ChimeraOS 仅维护用户级应用和插件功能。rEFInd 继续停用，Clover 通常由 UEFI GOP 自动选择分辨率，GPD WIN 3 会优先请求 1280x720 横屏模式；Bazzite 安装/修复 Clover 时会备份并清理检测到的旧 SteamOS 引导，但不会删除系统分区，其他通用 EFI 高风险工具不开放。
