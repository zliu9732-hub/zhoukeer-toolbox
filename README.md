...
- 微信、QQ：通过Flathub用户级安装，不修改SteamOS只读分区。
...
Steam Deck桌面模式会优先启动KDE原生图形菜单，可直接使用触屏或触控板点击分类和功能，不需要输入数字。执行安装、清理等操作前会显示可点击的确认按钮；只有管理员密码仍需在终端中输入。

如果系统缺少 `kdialog` 或当前没有图形桌面环境，工具箱会自动回退到左侧分类栏、右侧说明区的双栏终端界面。终端界面支持字母，也兼容数字 `1` 到 `7` 和 `0`。需要主动使用终端界面时可运行：
