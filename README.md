...
- 游戏与掌机助手：一键下载 Epic、战网和育碧官方 Windows 安装包，自动创建带工具箱标识的桌面入口，并写入当前 Steam 账号的非 Steam 游戏库及完整封面。小黄鸭和 FSR4 使用 GitHub 加速与 Gitee 国内归档双源，GitHub 全部失败后自动切换 Gitee。战网由 Steam 原生条目配合 Proton Experimental 完成安装并复用同一兼容环境；Epic 与育碧继续由工具箱自动准备兼容层，缺少时通过 Steam 补齐官方 Proton。
