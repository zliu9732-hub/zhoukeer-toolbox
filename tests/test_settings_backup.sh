...
grep -Fq 'TOOLBOX_NAME="周克儿工具箱"' "$CONFIG" || fail "工具箱配置未恢复"
grep -Fq 'zram-size = ram / 2' "$ZRAM" || fail "内存参数未恢复"
grep -Fq 'X-Zhoukeer-Managed=true' "$HOME_DIR/Desktop/Test.desktop" || fail "工具箱快捷方式未恢复"
