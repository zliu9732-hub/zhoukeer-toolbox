...
grep -Fq 'ICON_PATH="$INSTALL_DIR/assets/icon-round.png"' "$PROJECT_ROOT/install.sh"
grep -Fq 'bash "$INSTALL_DIR/modules/software.sh" browser' "$PROJECT_ROOT/install.sh"
grep -Fq 'GUI_ICON="$PROJECT_ROOT/assets/icon-round.png"' "$PROJECT_ROOT/core/gui.sh"
file "$PROJECT_ROOT/assets/icon-round.png" | grep -Fq 'PNG image data, 512 x 512, 8-bit/color RGBA'
