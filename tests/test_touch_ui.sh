...
printf '%s\n' "$main_disclaimer" | grep -Fq 'ColorScheme=ZhoukeerToolboxSplash' || fail "启动时没有重新应用完整免责声明图片主题"
printf '%s\n' "$main_disclaimer" | grep -Fq 'ZHOUKEER_STARTUP_SPLASH' || fail "免责声明没有区分图片启动页与文字回退"
