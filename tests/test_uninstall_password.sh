...
printf '密码：test-only\n' > "$TEST_ROOT/home/Desktop/密码.txt"
chmod 600 "$TEST_ROOT/home/Desktop/密码.txt"

# 确认卸载、不保留配置、不保留日志、明确删除密码记录。
printf 'y\nn\nn\ny\n' | HOME="$TEST_ROOT/home" bash "$TEST_ROOT/source/uninstall.sh" >/dev/null

if [ -e "$TEST_ROOT/home/Desktop/密码.txt" ] || [ -L "$TEST_ROOT/home/Desktop/密码.txt" ]; then
    echo "FAIL: 卸载时确认删除后仍遗留密码记录" >&2
    exit 1
fi

mkdir -p "$TEST_ROOT/home/Desktop/密码.txt"
printf 'y\nn\nn\ny\n' | HOME="$TEST_ROOT/home" bash "$TEST_ROOT/source/uninstall.sh" >/dev/null
if [ ! -d "$TEST_ROOT/home/Desktop/密码.txt" ]; then
