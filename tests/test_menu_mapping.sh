...
assert_contains "$touch_lsfg" 'right:8-9:v2' "小黄鸭子菜单缺少 2.0 坐标"
assert_contains "$touch_lsfg" 'right:11-12:mako' "小黄鸭子菜单缺少 MAKO 坐标"
assert_contains "$touch_lsfg" 'modules/plugin_store.sh" lsfg-zh-gitee' "小黄鸭 1.0 动作错误"
assert_contains "$touch_lsfg" 'modules/plugin_store.sh" lsfg-v2' "小黄鸭 2.0 动作错误"
...
assert_contains "$touch_handheld_plugins" 'right:19-20:onexplayer-tools' "掌机控制子菜单缺少 OneXPlayer 机型工具坐标"
...
assert_contains "$touch_dual_more" 'modules/dual_system_tools.sh" repair-boot' "双系统触控引导修复未调用 NVRAM 修复模块"
assert_contains "$gui_dual_more" 'modules/dual_system_tools.sh" repair-boot' "双系统 GUI 引导修复未调用 NVRAM 修复模块"
...
assert_contains "$gui_dual_more" 'previous "上一页：返回常用工具' "双系统 GUI 更多工具页缺少上一页入口"
assert_contains "$gui_dual" 'dual_system_more_menu' "双系统 GUI 下一页没有进入独立子菜单"
assert_contains "$gui_dual_more" 'previous) return 0' "双系统 GUI 上一页没有返回常用工具"
...
for file in "$MAIN_FILE" "$GUI_FILE"; do
    ...
    assert_contains "$source_text" 'modules/onexplayer_button_fix.sh" reboot' "壹号掌机修复立即重启动作错误：$file"
    ...
    assert_contains "$source_text" 'modules/dual_system_tools.sh" repair-boot' "双系统 NVRAM 修复动作错误：$file"
    assert_contains "$source_text" 'modules/clover_boot.sh" hide-menu' "Clover 隐藏菜单动作错误：$file"
    assert_contains "$source_text" 'modules/clover_boot.sh" show-menu' "Clover 菜单恢复动作错误：$file"
    assert_contains "$source_text" 'modules/clover_boot.sh" default-windows' "Clover 默认 Windows 动作错误：$file"
    assert_contains "$source_text" 'modules/clover_boot.sh" default-steamos' "Clover 默认 SteamOS 动作错误：$file"
