...
for file in "$MAIN_FILE" "$GUI_FILE"; do
    ...
    assert_contains "$source_text" 'modules/f1l_button_fix.sh" install' "飞行家 F1L 按键修复安装动作错误：$file"
    assert_contains "$source_text" 'modules/f1l_button_fix.sh" status' "飞行家 F1L 按键修复状态动作错误：$file"
    assert_contains "$source_text" 'modules/f1l_button_fix.sh" restore' "飞行家 F1L 按键恢复动作错误：$file"
