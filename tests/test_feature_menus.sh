...
assert_contains "$touch_plugin_page_2" '启动器与封面' "插件第二页缺少启动器与封面子菜单"
assert_contains "$touch_launcher_tools" '战网与黑盒工坊' "启动器与封面子菜单缺少战网入口"
...
assert_contains "$touch_handheld_plugins" 'OneXPlayer 机型工具' "掌机控制插件菜单缺少 OneXPlayer 机型工具"
assert_contains "$touch_handheld_plugins" 'X2 Mini Pro 亮度修复（已实测）' "掌机控制插件菜单缺少 X2 Mini Pro 实测标注"
assert_contains "$touch_onexplayer_tools" 'X2 Mini Pro 亮度修复' "OneXPlayer 子菜单缺少 X2 Mini Pro 亮度修复"
assert_contains "$touch_onexplayer_tools" 'ONEXPLAYER X2 Mini Pro' "X2 Mini Pro 亮度修复缺少实测机型"
assert_contains "$touch_onexplayer_tools" 'modules/plugin_store.sh" lego2-brightness-fix' "X2 Mini Pro 亮度修复未调用独立安装动作"
assert_contains "$touch_onexplayer_tools" 'OneXPlayer Apex 工具' "OneXPlayer 子菜单缺少 Apex 工具"
assert_contains "$touch_onexplayer_tools" 'modules/plugin_store.sh" onexplayer-apex' "OneXPlayer Apex 工具未调用独立安装动作"
assert_contains "$touch_onexplayer_tools" '原版 SteamOS' "OneXPlayer Apex 工具缺少 SteamOS 适用范围提示"
assert_contains "$gui_games" 'X2 Mini Pro 亮度修复' "GUI 缺少 X2 Mini Pro 亮度修复"
assert_contains "$gui_games" 'modules/plugin_store.sh" lego2-brightness-fix' "GUI X2 Mini Pro 亮度修复未调用独立安装动作"
...
assert_contains "$touch_games" 'right:14-15:handheld-pink-install' "Decky Loader 子菜单缺少 掌机 Pink 安装坐标"
assert_contains "$gui_games" 'rog-white-install' "GUI 插件商城缺少 ROG White 白色主题入口"
assert_contains "$gui_games" 'handheld-pink-install' "GUI 插件商城缺少 掌机 Pink 粉色主题入口"
assert_contains "$gui_games" 'modules/rog_white_theme.sh" install' "GUI ROG White 白色主题动作错误"
assert_contains "$gui_games" 'modules/handheld_pink_theme.sh" install' "GUI 掌机 Pink 粉色主题动作错误"
...
for menu in "$touch_f1" "$gui_f1"; do
    for item in '安装屏幕修复' '屏幕修复状态' '卸载屏幕修复' '安装特殊按键修复' '特殊按键修复状态' '恢复特殊按键修复' '更新 InputPlumber' '官方 0.79.2' '准备 V1.14 BIOS' '立即重启 SteamOS' 'F1L 仅限 F1 8840U' '5秒自动重启' 'ONEXPLAYER F1' '不使用 sudo'; do
        assert_contains "$menu" "$item" "飞行家 F1 子菜单缺少：$item"
    done
    assert_contains "$touch_f1" 'right:2-3:install' "飞行家 F1 屏幕修复安装坐标错误"
    assert_contains "$touch_f1" 'right:8-9:button-install' "壹号掌机按键修复安装坐标错误"
    assert_contains "$touch_f1" 'right:10-11:button-status' "壹号掌机按键状态坐标错误"
...
assert_contains "$touch_f1" '等待5秒自动重启SteamOS' "触控菜单没有提前说明自动重启"
assert_contains "$touch_f1" 'confirm_and_run "恢复壹号掌机 SteamOS 特殊按键修复"' "触控菜单的特殊按键恢复缺少单次确认"
assert_contains "$touch_f1" '完成后5秒自动重启' "触控菜单的特殊按键恢复没有自动重启说明"
...
assert_contains "$gui_f1" 'ZHOUKEER_AUTO_CONFIRM=1' "GUI 确认后未向模块传递授权"
...
assert_contains "$touch_games_page_1" 'right:11-12:cheatdeck' "CheatDeck 未放在插件第一页"
assert_contains "$touch_games_page_2" 'right:9-10:handheld-plugins' "掌机控制插件子菜单未放在插件第二页"
touch_handheld_plugins="$(function_source "$MAIN_FILE" handheld_plugins_menu)"
touch_onexplayer_tools="$(function_source "$MAIN_FILE" onexplayer_tools_menu)"
...
assert_contains "$touch_handheld_plugins" 'right:19-20:onexplayer-tools' "掌机控制插件子菜单缺少 OneXPlayer 机型工具"
assert_contains "$touch_onexplayer_tools" 'right:7-8:x2-mini-pro-brightness' "OneXPlayer 子菜单缺少 X2 Mini Pro 亮度修复"
