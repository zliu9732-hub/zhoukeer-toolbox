...
for i in $(seq 0 $((DOWNLOAD_PARTS - 1))); do
    part=$(printf '%02d' "$i")
    curl -fL "${MIRROR_BASE}/PluginLoader-pre.part.${part}" >> "${HOMEBREW_FOLDER}/services/PluginLoader" || { echo "PluginLoader 下载失败，请重试"; exit 1; }
done
...
curl -L https://gitee.com/zliu9732-hub/zhoukeer-toolbox/raw/main/decky-installer-cn/plugin_loader-prerelease.service  --output ${HOMEBREW_FOLDER}/services/plugin_loader-prerelease.service
